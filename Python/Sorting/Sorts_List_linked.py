...
from List.List_linked import List
...
class Sorts:
    ...
    def _partition(source):
        ...
        while source._front is not None:
            ...
            if pivot._value > source._front._value: #type: ignore
                lesser._move_front_to_rear(source)
            elif pivot._value < source._front._value: #type: ignore
        ...
        if a is not None:
            ...
            for i in range(max_val):
                while not a.is_empty():
                    cur_num = a._front._value
                    cur_digit = (cur_num % (10 ** (i + 1))) // (10 ** i)
                    lst_val[cur_digit]._move_front_to_rear(a)
                for i in lst_val:
                    if not i.is_empty():
                        a._append_list(i)
        return